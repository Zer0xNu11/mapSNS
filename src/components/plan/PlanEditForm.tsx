...
const PlanEditForm: React.FC<PlanEditFormProps> = ({ plan }) => {
  const currentPath = `${process.env.NEXT_PUBLIC_BASE_URL}/home/plans`;
  const [titleText, setTitleText] = useState(`${plan.title}`);
  ...
  const initialState: PlanEditFormState = {
    error: "",
    planId: plan.id,
    path: currentPath,
  };
  const [state, formAction] = useFormState(updatePlan, initialState);
